...
class ConvergenceManager {
 ...
 public:
  ...
  common::TizenResult CreateLocalService(const int serviceType,
 ...
 private:
  conv_device_h getRemoteDevice(const char *deviceId) const;
  conv_service_h getRemoteService(const conv_device_h device, const int serviceType) const;
  conv_service_h getService(const char *deviceId, const int serviceType) const;
  void destroyService(conv_service_h service);
  bool isLocalDevice(const char *deviceId) const;

 private:
  class ConvergenceInstance *plg;
  conv_h conv_;
  mutable std::unordered_map<std::string, conv_device_h> devices;
  mutable std::unordered_map<int, conv_service_h> __localServices;
};
