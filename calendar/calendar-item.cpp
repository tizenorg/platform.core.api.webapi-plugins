...
const PlatformPropertyMap CalendarItem::platform_property_map_ = {
    ...
    // event only
    {"endDate_time", {
        { CALENDAR_BOOK_TYPE_EVENT, _calendar_event.end_time }
    }},
    {"endDate_tzid", {
        { CALENDAR_BOOK_TYPE_EVENT, _calendar_event.end_tzid }
    }},
    {"recurrence_id", {
        { CALENDAR_BOOK_TYPE_EVENT, _calendar_event.recurrence_id }
    }},
    {"availability", {
        { CALENDAR_BOOK_TYPE_EVENT, _calendar_event.busy_status }
    }},

    // task only
    {"dueDate_time", {
        { CALENDAR_BOOK_TYPE_TODO, _calendar_todo.due_time }
    }},
    {"dueDate_tzid", {
        { CALENDAR_BOOK_TYPE_TODO, _calendar_todo.due_tzid }
    }},
    {"completedDate", {
        { CALENDAR_BOOK_TYPE_TODO, _calendar_todo.completed_time }
    }},
    {"progress", {
        { CALENDAR_BOOK_TYPE_TODO, _calendar_todo.progress }
    }}
};
...
unsigned int CalendarItem::GetPlatformProperty(int type, const std::string& property) {
...
int CalendarItem::StringToPlatformEnum(const std::string& field, const std::string& value) {
...
std::string CalendarItem::PlatformEnumToString(const std::string& field, int value) {
...
void CalendarItem::SetString(int type, calendar_record_h rec, const std::string& property,
    ...
    const std::string& value = common::FromJson<json::String>(in, property.c_str());
...
std::string CalendarItem::GetString(int type, calendar_record_h rec, const std::string& property) {
    LoggerD("get: %s", property.c_str());

    return CalendarRecord::GetString(rec, GetPlatformProperty(type, property));
}

void CalendarItem::SetInt(int type, calendar_record_h rec, const std::string& property,
                          const json::Object& in, bool optional) {
...
void CalendarItem::SetInt(int type, calendar_record_h rec, const std::string& property, int value) {
    LoggerD("set: %s", property.c_str());
...
int CalendarItem::GetInt(int type, calendar_record_h rec, const std::string& property) {
    LoggerD("get: %s", property.c_str());

    return CalendarRecord::GetInt(rec, GetPlatformProperty(type, property));
}

void CalendarItem::SetEnum(int type, calendar_record_h rec, const std::string& property,
                           const json::Object& in, const std::string& enum_name) {
...
std::string CalendarItem::GetEnum(int type, calendar_record_h rec, const std::string& property,
...
void CalendarItem::SetDouble(int type, calendar_record_h rec, const std::string& property,
                             double value) {
    LoggerD("set: %s", property.c_str());
...
double CalendarItem::GetDouble(int type, calendar_record_h rec, const std::string& property) {
    LoggerD("get: %s", property.c_str());

    double value;
    int ret = calendar_record_get_double(rec, GetPlatformProperty(type, property), &value);
    if (CALENDAR_ERROR_NONE != ret) {
...
void CalendarItem::SetCaltime(int type, calendar_record_h rec, const std::string& property,
...
calendar_time_s CalendarItem::GetCaltime(int type, calendar_record_h rec,
                                         const std::string& property, bool throw_on_error) {
    LoggerD("get: %s", property.c_str());
...
calendar_time_s CalendarItem::GetCaltime(calendar_record_h rec, unsigned int property,
                                         bool throw_on_error) {
    calendar_time_s cal;
    ...
    if (property != -1u) {
        ...
        if (CALENDAR_ERROR_NONE != ret) {
            ...
            if (throw_on_error) {
                throw common::UnknownException("Can't get calendar_time value form record");
            }
        }
    }
...
long long int CalendarItem::GetLli(int type, calendar_record_h rec, const std::string& property) {
    LoggerD("get: %s", property.c_str());
...
Date CalendarItem::DateFromJson(const json::Object& in) {
    ...
    Date date = {
        (long long int) common::FromJson<double>(in, "UTCTimestamp"),
...
json::Value CalendarItem::DateToJson(Date date) {
    ...
    date_obj["UTCTimestamp"] = json::Value(static_cast<double>(date.utc_timestamp_));
...
void CalendarItem::CategoriesFromJson(int type, calendar_record_h rec, const json::Array& value) {
...
void CalendarItem::AttendeesFromJson(int type, calendar_record_h rec, const json::Array& value) {
    LoggerD("enter");
    ...
    for (auto& item : value) {
        ...
        if(!IsNull(obj, "delegatorURI")) {
            CalendarRecord::SetString(attendee, _calendar_attendee.delegator_uri,
                    common::FromJson<json::String>(obj, "delegatorURI"));
        }
        if(!IsNull(obj, "delegateURI")) {
            CalendarRecord::SetString(attendee, _calendar_attendee.delegatee_uri,
        ...
        if(!IsNull(obj, "contactRef")) {
            ...
            const std::string& address_book = common::FromJson<json::String>(obj,
...
json::Array CalendarItem::AttendeesToJson(int type, calendar_record_h rec) {
    ...
    if (CALENDAR_ERROR_NONE != calendar_record_clone_child_record_list(rec, property, &list)) {
    ...
    for (unsigned int i = 0; i < count; ++i) {
        ...
        attendee_obj["uri"] = json::Value(CalendarRecord::GetString(attendee,
                _calendar_attendee.email, false));

        attendee_obj["name"] = json::Value(CalendarRecord::GetString(attendee,
                _calendar_attendee.name, false));

        attendee_obj["role"] = json::Value(GetEnum(attendee, _calendar_attendee.role,
                kAttendeeRole));

        attendee_obj["status"] = json::Value(GetEnum(attendee, _calendar_attendee.status,
                kAttendeeStatus));

        attendee_obj["RSVP"] = json::Value((bool)CalendarRecord::GetInt(attendee,
                _calendar_attendee.rsvp, false));

        attendee_obj["type"] = json::Value(GetEnum(attendee, _calendar_attendee.cutype,
                kAttendeeType));

        attendee_obj["group"] = json::Value(CalendarRecord::GetString(attendee,
                _calendar_attendee.group, false));

        attendee_obj["delegatorURI"] = json::Value(CalendarRecord::GetString(attendee,
                _calendar_attendee.delegator_uri, false));

        attendee_obj["delegateURI"] = json::Value(CalendarRecord::GetString(attendee,
                _calendar_attendee.delegatee_uri, false));

        // contactRef
        const std::string& contact_id = CalendarRecord::GetString(attendee,
                _calendar_attendee.uid, false);
        int book_id = CalendarRecord::GetInt(attendee, _calendar_attendee.person_id, false);
        attendee_obj["contactRef"] = json::Value(json::Object{
            {"contactId", json::Value(contact_id)},
...
void CalendarItem::AlarmsFromJson(int type, calendar_record_h rec,
    ...
    for (auto& item : alarms) {
        ...
        if (!common::IsNull(obj, "before")) {
            ...
            const std::string& unit = common::FromJson<json::String>(obj, "before", "unit");
            long long tick = 0;
        ...
        CalendarRecord::SetString(alarm, _calendar_alarm.description,
...
json::Array CalendarItem::AlarmsToJson(int type, calendar_record_h rec) {
    ...
    for (unsigned int i = 0; i < count; ++i) {
        ...
        if (tick_unit == CALENDAR_ALARM_TIME_UNIT_SPECIFIC) {
            ...
            alarm_obj["before"] = json::Value(json::Object{
                {"length", json::Value(static_cast<double>(length))},
                {"unit", json::Value(unit)}
            });
        }

        alarm_obj["method"] = json::Value(GetEnum(alarm, _calendar_alarm.action,
                kAlarmMethod));

        alarm_obj["description"] = json::Value(CalendarRecord::GetString(alarm,
                _calendar_alarm.description, false));
...
void CalendarItem::RecurrenceRuleFromJson(calendar_record_h rec, const json::Object& rrule) {
    LoggerD("enter");

    const std::string& frequency = common::FromJson<json::String>(rrule, "frequency");
    ...
    if (-1 != occurrence_count) {
        ...
        CalendarRecord::SetInt(rec, _calendar_event.range_type, CALENDAR_RANGE_COUNT);
    }
    ...
    if (!common::IsNull(rrule, "untilDate")) {
        ...
        CalendarRecord::SetInt(rec, _calendar_event.range_type, CALENDAR_RANGE_UNTIL);
    }

    const json::Array& byday_array = common::FromJson<json::Array>(rrule, "daysOfTheWeek");
    std::string byday;
    ...
    const json::Array& bysetpos_array = common::FromJson<json::Array>(rrule, "setPositions");
    std::string bysetpos;
    ...
    CalendarRecord::SetString(rec, _calendar_event.exdate,
...
json::Object CalendarItem::RecurrenceRuleToJson(calendar_record_h rec) {
    ...
    out["frequency"] = json::Value(GetEnum(rec, _calendar_event.freq, kRecurrenceRuleFrequency));
    ...
    out["daysOfTheWeek"] = json::Value(StringToArray(
            CalendarRecord::GetString(rec, _calendar_event.byday)));

    out["setPositions"] = json::Value(StringToArray(
            CalendarRecord::GetString(rec, _calendar_event.bysetpos)));

    const json::Array& exceptions = StringToArray(
        CalendarRecord::GetString(rec, _calendar_event.exdate));
    json::Array dates = json::Array();
    for (auto& exception : exceptions) {
        Date date = {
            common::stol(exception.get<std::string>()),
            0, 0, 0, ""
        };
...
Date CalendarItem::DateFromPlatform(int type, calendar_record_h rec, const std::string& property) {
    LoggerD("enter");
...
Date CalendarItem::DateFromPlatform(calendar_record_h rec, unsigned int property) {
    LoggerD("enter");
...
void CalendarItem::FromJson(int type, calendar_record_h rec, const json::Object& in) {
    LoggerD("enter");
    ...
    if (!common::IsNull(in, endProperty.c_str())) {
        ...
        SetCaltime(type, rec, endProperty + "_time", DateToPlatform(end, is_all_day));
    ...
    CategoriesFromJson(type, rec, common::FromJson<json::Array>(in, "categories"));
    ...
    if (type == CALENDAR_BOOK_TYPE_EVENT) {
        ...
        if (!common::IsNull(in, "recurrenceRule")) {
            RecurrenceRuleFromJson(rec, common::FromJson<json::Object>(in, "recurrenceRule"));
        }
...
void CalendarItem::ToJson(int type, calendar_record_h rec, json::Object* out_ptr) {
    LoggerD("enter");
    ...
    out["lastModificationDate"] = json::Value(static_cast<double>(
            GetLli(type, rec, "lastModificationDate")));
    ...
    out["visibility"] = json::Value(GetEnum(type, rec, "visibility", kItemVisibility));
    ...
    if (type == CALENDAR_BOOK_TYPE_EVENT) {
        ...
        out["priority"] = json::Value(GetEnum(type, rec, "priority", kEventPriority));
        out["availability"] = json::Value(GetEnum(type, rec, "availability", kEventAvailability));
        out["recurrenceRule"] = json::Value(RecurrenceRuleToJson(rec));
    } else {
        out["status"] = json::Value(GetEnum(type, rec, "status", kTaskStatus));
        out["priority"] = json::Value(GetEnum(type, rec, "priority", kTaskPriority));

        out["completedDate"] = json::Value(static_cast<double>(
                GetLli(rec, _calendar_todo.completed_time)));
        out["progress"] = json::Value(static_cast<double>(GetInt(type, rec, "progress")));
    }
}
