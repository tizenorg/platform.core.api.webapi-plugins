...
std::string CalendarRecord::GetString(calendar_record_h rec, unsigned int property,
                                      bool throw_on_error) {
    char* value = NULL;
...
int CalendarRecord::GetInt(calendar_record_h rec, unsigned int property, bool throw_on_error) {
    int value;
...
void CalendarRecord::SetInt(calendar_record_h record, unsigned int property, int value,
                          bool throw_on_error) {
    LoggerD("enter");
...
CalendarRecordPtr CalendarRecord::GetById(int id, const char* view_uri) {
    ...
    if (CALENDAR_ERROR_NONE != ret || nullptr == handle) {
        LoggerE("Fail to get calendar record %d for view %s, error code: %d", id, view_uri, ret);
...
void CalendarRecord::AddChildRecord(calendar_record_h rec, unsigned int property,
...
void CalendarRecord::RemoveChildRecords(calendar_record_h rec, unsigned int property_id) {
    ...
    for (unsigned int i = 0; i < count; ++i) {
        ...
        if (CALENDAR_ERROR_NONE != calendar_record_get_child_record_at_p(rec, property_id, 0, &attendee)) {
...
unsigned int CalendarRecord::GetChildRecordCount(calendar_record_h rec, unsigned int property,
                                                 bool throw_on_error) {
    unsigned int value;
...
bool CalendarRecord::GetChildRecordAt(calendar_record_h rec, unsigned int property,
                                      calendar_record_h* result, int index, bool throw_on_error) {
...
void CalendarRecord::CalendarToJson(calendar_record_h rec, json::Object* out_ptr) {
    ...
    out.insert(std::make_pair("accountId", json::Value(std::to_string(account_id))));
    out.insert(std::make_pair("name", json::Value(name)));
    out.insert(std::make_pair("type", json::Value(type)));
}

void CalendarRecord::CalendarFromJson(calendar_record_h rec, const json::Object& in) {
    if (in.empty()) {
